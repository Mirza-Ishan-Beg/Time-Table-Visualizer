...
## Introduction
The program generates high resolution images using a Time Table in Excel file. The generated visual is a 24-hour clock that the matplotlib generates using the polar coordinate system. The divisions within the visuals are also well separated, hourly separations are a line, while few minutes (for instance, 5 min. intervals) are a faint dashed lines.

The excel file is parsed using regex so merely getting a pattern of time snippets to match is enough, and allows some limited level of flexibility so far.

## Back Story [SkipToInstallation](#Installation)
